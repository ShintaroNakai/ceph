...
void PG::fulfill_log(int from, const Query &query, epoch_t effective_epoch)
{
  assert(!acting.empty());
  assert(from == acting[0]);
  assert(query.type != PG::Query::INFO);
...
bool PG::old_peering_msg(epoch_t msg_epoch,
			 epoch_t effective_epoch)
{
  return (last_warm_restart > msg_epoch ||
	  last_warm_restart > effective_epoch);
}
...
PG::RecoveryState::ReplicaActive::react(const MQuery& query) {
  ...
  pg->fulfill_log(query.from, query.query, query.effective_epoch);
  return discard_event();
}
				     ...
				     RecoveryCtx *rctx)
{
  ...
  start_handle(rctx);
  machine.process_event(MQuery(from, q, effective_epoch));
  end_handle();
}
