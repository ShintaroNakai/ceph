...
public:
  ...
  {
    dout(20) << "=========== adding new interval :{%s}===========" << in << dendl;
...
#ifdef DEBUG
    dout(20) << "tree after insert node: " << dendl;
    printTree(std::cout);
#endif
  ...
  {
    dout(20) << "searching for interval  {" <<  left << "-" << right <<"}" <<dendl;
  ...
  void removeInterval(const value_type& start, const value_type& end, const T& in)
  {
    ...
    for(i = dispList.begin(); i  != iEnd; ++i) {
      dout(20) << "relocating node " << (*i).node << dendl;
      splitIntervalNode((*i), range, splitNodes);
      jEnd = splitNodes.end();
      for(j = splitNodes.begin(); j != jEnd; ++j) {
	dout(20) << "split node " << (*j).node << "{ " << (*j).start << "-" << (*j).end <<"}" << dendl;
	insert((*j).start);
	insert((*j).end);
  ...
  void removeAll(const T& in)
  {
    ...
    if(!delList.size()) {
      dout(20) << "no nodes gets deleted" << dendl;
    }
...
private:
  ...
  void doIterativeRebalance(TreeNode* node)
  {
    ...
    while(node) {
      node->adjustHeight();
      dout(20) << "rebalancing" << toString(node) << dendl ;
      node = rebalanceTree(node, dispList);
      dout(20) << "after rotation.... current root :" << toString(node) << dendl;
      last = node;
      node = node->parent;
    }
    root = last;
  ...
  void insertNodeList(std::list<IntervalNode>& nodeList)
  {
    ...
    while(i != nodeList.end()) {
      dout(20) << "inserting displaced node" << (*i).node << dendl;
  ...
  {
    assert(root != NULL);
    dout(20) << "removing ' " << value << " ' range :{"  << range.start << "-"  << range.end << "}" << dendl;
    ...
    while(current) {
      dout(20) << "current :"<< toString(current) << dendl;
	...
	if(isInterSect(range.start, range.end, currRange.start, currRange.end)) {
      ...
      dout(20) << "removing displaced lists. current list size :" << current->rangeList.size() << dendl;
      current->rangeList.remove_if(pred);
      dout(20) << "after removal... list size :" << current->rangeList.size() << dendl;
  ...
  {
#ifdef DEBUG
    dout(20) << "checking range of " << toString(node) << dendl;
  ...
  {
    dout(20) << "checking eligiblity of node : " << toString(node) << "for cleanup" << dendl;
    if(node == NULL) {
      return false;
    }
  ...
  {
    dout(20) << "search val : " << searchVal << ", interval :{" << range.start << "-" << range.end << "}," << "removing :" << in << dendl;
  ...
  void deleteIntervalNode(TreeNode* node, void* data)
  {
    ...
    dout(20) << "deleting data node :" << arg->node << "in current " <<  toString(node) <<dendl;
    ...
    if(isEligibleForCleanup(node)) {
      dout(20) << "node eligible for cleanup" <<dendl;
  ...
  void splitIntervalNode(const IntervalNode& inode, const IntervalRange& range,  std::list<IntervalNode>& nodeList)
  {
    ...
    if((inode.start < range.start) || (inode.end > range.end)) {
      dout(20) << "splitting node " << inode.node << dendl;
	...
	if(inode.end > range.end) {
	  ...
	  dout(20) << "case 1 for node :" << inode.node << " interval node :{" <<  inode.start <<  " - " <<inode.end << "}" <<dendl;
	  ...
	  dout(20) << "case 2 for node :" << inode.node << " interval node :{" <<  inode.start <<  " - " <<inode.end << "}" <<dendl;
	  newNode.start = inode.start;
	  newNode.end = range.start;
	  nodeList.push_back(newNode);
	}
	...
	//	range
	dout(20) << "case 3 for node :" << inode.node << " interval node :{" <<  inode.start <<  " - " <<inode.end << "}" <<dendl;
	newNode.start = range.end;
	newNode.end = inode.end;
	nodeList.push_back(newNode);
      }
  ...
  {
    dout(20) << "linking parent of " << toString(node) << "   to "<<  toString(child) << dendl;

    if(node->parent) {
      if(node->parent->left == node) {
	dout(20) << "linking to parent " <<  toString(node->parent) << " 's left" <<dendl;
	node->parent->left = child;
      }
      else if(node->parent->right == node) {
	dout(20) << "linking to parent " <<  toString(node->parent) << " 's right" <<dendl;
  ...
  IntervalTree::TreeNode* deleteNode(IntervalTree::TreeNode* node, std::list<IntervalNode>& dispList)
  {
    ...
    dout(20) << "deleting node " << toString(node) <<dendl;
#ifdef DEBUG
    dout(20) << "disp nodes before delete :" << dendl;
    typename std::list<IntervalNode>::iterator j;
    for(j = dispList.begin() ; j != dispList.end(); ++j) {
      dout(20) << "node :" <<  (*j).node << dendl;
    }
#endif
    TreeNode* parent;
    ...
    if((node->left == NULL) && (node->right == NULL)) {
      dout(20) <<"left and right child are absent" << dendl;
      parent = node->parent;
    ...
    //
      ...
      dout(20) << "left or right child is absent" << dendl;
      parent = node->parent;
      if(node->left != NULL) {
	dout(20) << "left child present" << dendl;
	TreeNode* prevNode = node->spanNodes.first;
	...
	adjustSpan(prevNode);
      ...
      if(node->right != NULL) {
	dout(20) << "right child present" << dendl;
	TreeNode* nextNode = node->spanNodes.second;
    ...
    //
      ...
      dout(20) << "both left and right child present" << dendl;
	...
	nextNode->rangeList.clear();
      ...
      dout(20) << "next node :" << toString(nextNode) << dendl;
  ...
  {
    dout(20) << "searching for value :" << value <<dendl;
  ...
  void insertNodeInRange(TreeNode* current, const IntervalNode& in, IntervalRange& spanRange)
  {
    ...
    dout(20) << "spanRange :{" << spanRange.start << "-" <<   spanRange.end <<"}" << dendl;
    i = std::find(current->rangeList.begin(), current->rangeList.end(), spanRange);
#ifdef DEBUG
    dout(20) << "current nodes :" <<dendl;
    current->printNodes(std::cout);
#endif
    if(i == current->rangeList.end()) {
      dout(20) << " could not find the range .. will be adding now" << dendl;
      spanRange.intervalNodeSet.push_back(in);
      current->rangeList.push_back(spanRange);
    }
    else {
      dout(20) << " range already in the list" << dendl;

      dout(20) << "adding node " << in.node << "to the range list" << dendl;
  ...
  void insertNode(const IntervalNode& in, const value_type& value)
  {
    ...
    while(current) {
      dout(20) << "inserting node :" << in.node << " , value : " <<value << " ,  at current :" << toString(current)<< dendl;

      std::pair<TreeNode* ,TreeNode*> span = getSpan(current);
      prevNode = prev(current);
      prevNode = prevNode == NULL ? current : prevNode;
      dout(25) << "prev node :" << toString(prevNode) << dendl;

      nextNode = next(current);
      nextNode = nextNode == NULL ? current : nextNode;
      dout(25) << "next node :" << toString(nextNode) << dendl;

      dout(25) << "span for node " << toString(current) <<" : {" <<  toString(span.first) << " - " << toString(span.second) << "}" <<
	...
	dout(25) << "span : {" << spanPrev->value << " - " << spanNext->value << "}" << dendl;
	if(spanPrev->value >= in.start && spanNext->value <= in.end) {
	  dout(25) << "inserting over the full span " << dendl;
	...
	dout(20) << "span :{" << span.first->value << " - " << span.second->value << "}" << dendl;
  ...
  void insert(const value_type& in)
  {
    ...
    dout(20) << "adding new tree node :" << toString(newNode) <<dendl;
    ...
    while(current) {
      dout(20) << "current node :" << toString(current) <<dendl;
      if(current->value == in) {
	dout(20) << "a node already exists with same value.. abandoning!!\n"  <<dendl;
	current->refCount++;
	break;
      }
	...
	if(current->left == NULL) {
	  dout(20) <<  "inserting new node " << toString(newNode)  << " to LEFT"  << dendl;
	...
	if(NULL == current->right) {
	  dout(20) <<  "inserting new node "  << toString(newNode)  << " to RIGHT"  << dendl;
...
#ifdef DEBUG
    dout(20) << "@@ tree after insert " << dendl;
    printTree(std::cout);
#endif
  }
  ...
  {
    dout(25) << "ax1 :" << ax1 << ", ax2:" << ax2 << ", bx1 :" << bx1 << ", bx2 :" << bx2 << dendl;
    if(
      (ax1 >= bx1 && ax1 < bx2)
      || (ax2 > bx1 && ax2 <= bx2)
      || (ax1 <= bx1 && ax2 > bx2)
      ) {
      dout(25) << "intersects!! " << dendl;
      return true;
    }
  ...
  TreeNode* doLeftRotation(TreeNode* n)
  {
    ...
    if(n->right == NULL) {
      dout(20) << "no right child. cant do left rotation" <<dendl;
      return n;
    }

    dout(20) << "left rotating " << toString(n->right) << " on pivot " <<  toString(n) <<dendl;
    ...
    dout(20) << "returning root :" << toString(root) <<dendl;
    return root;
  }
  ...
  {
    if(n == NULL) {
      dout(20) << "cant left rotate a null node" <<dendl;
      return n;
    }

    if(n->left == NULL) {
      dout(20) << "no left child. cant do right rotation" <<dendl;
      return n;
    }

    dout(20) << "right rotating " <<  toString(n->left) << "on pivot :" << toString(n) << dendl;
    ...
    dout(20) << "after rotate root->right :" <<  toString(root->right) <<dendl;
    dout(20) << "returning root : " <<  toString(root) <<dendl;
    return root;
  }
  ...
  void adjustSpanAndDisplace(TreeNode* current,  std::list<IntervalNode>& dispList)
  {
    ...
    if(current->rangeList.size()) {
      ...
      for(i = current->rangeList.begin(); i != iEnd; ++i) {
	IntervalRange& range = *i;
	dout(20) << "range :{" << range.start << "-" << range.end << "}" << dendl;
	...
	if(span.first != NULL  && span.second != NULL) {
	  ...
	  if((range.start != spanPrev->value && range.start != current->value) || (range.end !=spanNext->value && range.end != nextNode->value)) {
	    dout(20) << "spanPrev :' " << spanPrev->value << " ' , spanNext :' " <<
	...
	if(isSpliceRequired) {
	  dout(20) << "splicing range :{" << range.start << " - " << range.end <<"}" <<dendl;
#ifdef DEBUG
	  range.printRange(std::cout);
#endif
	  dispList.splice(dispList.end(), range.intervalNodeSet);
	  dout(20) << "after splicing setsize:" << range.intervalNodeSet.size() <<dendl;
	}
      }
      dispList.sort();
      dispList.unique();
      ...
      RemoveIntervalRangePredicate pred;
      dout(25) << "removing displaced lists. current list size :" << current->rangeList.size() << dendl;
      current->rangeList.remove_if(pred);
      dout(25) << "range list size after cleanup:" <<  current->rangeList.size() << dendl;
    }
  }
  ...
  TreeNode* rebalanceTree(TreeNode* p,  std::list<IntervalNode>& dispList)
  {
    ...
    dout(20) << "rebalancing  :" << toString(p) <<dendl;
    ...
    if(p->getBalance() <= -2) {
      dout(20) << "right side heavy" <<dendl;
      if((p->right != NULL) && (p->right->getBalance() <= -1)) {
	dout(20) << "right child is right heavy" << dendl;
	...
	dout(20) << "right child is left heavy" <<dendl;
	...
	root = doLeftRotation(root);
      ...
      dout(20) << "left side heavy" << dendl;
      if((p->left != NULL) && (p->left->getBalance() >= 1)) {
	dout(20) << "left child is left heavy" <<dendl;
	...
	dout(20) << "left child is right heavy" <<dendl;
    ...
    if(root->parent != NULL && isLeft) {
      dout(20) << "root : " <<  toString(root) << " parent : " << toString(root->parent)
  ...
  IntervalTree::TreeNode* prev(TreeNode* node)
  {
	...
	next = next->right;
      ...
      dout(25) <<"returning prev :" << toString(parent) << dendl;
      return parent;
    }

    parent = node->parent;
    if(parent == NULL) {
      dout(25) <<"returning prev :" << toString(node) << dendl;
      return node;
    }
    next = node;
    while(parent) {
  ...
  std::pair<IntervalTree::TreeNode*, IntervalTree::TreeNode*> getSpan(const TreeNode* node) const
  {
    ...
    dout(25) << "returning span :{" << toString(lastLeftNode) << " , " << toString(lastRightNode) << "}" << dendl;
